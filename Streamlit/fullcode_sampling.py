...
import time  # Adicionado para medir o tempo de carregamento
# Tempo de carregamento do dataset: 79.20 segundos
...
def load_data():
    ...
    dfs = [pd.read_excel('..\\Dados\\PESQUISA DE ART 2022 2023 2024.xlsx', sheet_name=f"{i}", header=7, nrows=1000).dropna() for i in years]
...
dfs[1].rename(columns={'DATA REGISTRO': 'CIDADE'}, inplace=True)
dfs[1].rename(columns={'TITULOS': 'DATA REGISTRO'}, inplace=True)
dfs[1].rename(columns={'ATIVIDADES': 'TITULOS'}, inplace=True)
dfs[1].rename(columns={'OBSERVACAO OBRA SERVICO': 'ATIVIDADES'}, inplace=True)
dfs[1].rename(columns={'Unnamed: 6': 'OBSERVACAO OBRA SERVICO'}, inplace=True)
...
# Localização: Esta função é responsável por gerar o gráfico de barras horizontais
# com o padrão visual atualizado do CREA-MG, incluindo fundo branco e configurações específicas.
